...
I looked at the drug distribution over time for each district. They look very different. For example, TENDERLOIN (top) has been historically dominated by CRACK-related incidents (though this is changing, more on that later). PARK (which is basically Haight-Ashbury) is mostly MARIJUANA-related incidents.
...
If you back to the data at top, it the total number of drug-related incidents has been dropping. The mix is becoming more equally distributed between CRACK, MARIJUANA, and METH. But, there's a lot more to look at with this data. I invite folks to pull the [notebook](http://nbviewer.ipython.org/github/lmart999/GIS/blob/master/SF_GIS_Crime.ipynb) and explore for themselves!
