...
Data pipelines have producers and consumers. Producers write logs that may contain web events or physical data (e.g., RFID and IoT). Consumers process the logs. Because event data tends to be several orders of magnitude larger than traditional database uses, the structure of data pipelines have become [complex](http://www.slideshare.net/charmalloc/current-and-future-of-apache-kafka).
...
To address this, Jay and colleagues developed a central log that (1) acts as a buffer between producers and consumers, (2) allows for subscription by systems (e.g., Hadoop) that consume data at different rates, and (3) means that new data systems (source or destination) can be connected with a single pipeline.
