...
A key challenge is that the feedback loop between product/features and the metrics is not always obvious. The outside world also has an effect on metrics! Controlled experiments can be used to isolate the effects of a product/features on a metric from all of the external factors that could also influence the metric.
...
It's also critical to consider the funnel from user entry to conversion, which may have many steps. Each step may introduce frictions that affect the metric! [Jan Overgoor](http://nerds.airbnb.com/experiments-airbnb/), a data scientist at AirBnB, has a great blog post that discusses this: in addition to capturing external factors, experiments can be partitioned by specific steps in the funnel.

<figure>
<img src="/assets/funnel_airbnb.png" width="80%">
<figcaption>
</figcaption>
</figure>
...
He talks about a failed experiment at Etsy: it involved a "monolithic" roll-out of a new features (the infinite scroll that automates loading of new items). Surprisingly, this lead to fewer impressions, fewer items seen, and fewer purchases. When it failed, it was hard to de-bug because so much had been changed at once. 

A set of smaller, carefully designed experiments showed that the basic premises (e.g., more and faster results are good) were wrong. He goes on to discuss to discuss a more successful product launch in which carefully staged experiments gave them clear opportunity to test hypotheses and, if needed, alter course after each one.
