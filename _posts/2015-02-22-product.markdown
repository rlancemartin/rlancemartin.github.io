...
Different markets can complicate this. Chamath [talks](https://www.youtube.com/watch?v=raIUQP71SBU) about Facebook's international growth and need for localization of the app to specific markets. 

Uber is doing this now. They are in 150 cities and 46 counties. Gross revenue has more than doubled for the past three years. This is impressive, but their distribution of revenue is also interesting: apparently the bulk comes from [~ 10 cities](http://www.businessinsider.com/ubers-revenue-2014-11). There's a lot of headroom. This also shows that the business has high heterogeneity.

Part of the heterogeneity is due to the cities themselves, such as specific demographic or structural features for each one. High-resolution city-specific features are accesible, in some cases. 

For example, I pulled all [311 requests](http://www.sf311.org/index.aspx?page=161) provided by `SF open data`. This contains 1.1 million service requests collected across ~900 request categories over the past few years. What's cool is that each record is geo-tagged. So, I used the [`basemap`](http://matplotlib.org/basemap/) library in `python` to overlay reported `Pavement Defects` on a map of San Francisco. 
...
Along with this available city data, Uber has organized their own data into city- and neighborhood-specific features. For example, neighborhoods have [unique usage patterns](http://www.citylab.com/commute/2012/08/using-uber-ridership-data-compare-cities-and-neighborhoods/2890/). This means that each city and neighborhood can be understood as a potentially broad set of granular features.

So what? These features provide a handle for organizing the heterogeneity such that it can understood in a few obvious ways.

* *** Comparative analysis *** Neighborhoods or cities can be grouped based upon similar features. This grouping isn't necessarily intuitive, which is why algorithms to do this can be useful. Grouping can identify outliers, where to expand next, and it may identify ways to apply insights from one market to similar ones.
...
* *** Localizing growth strategy *** I wonder how much market-specific features explain the variance in key (user or driver) metrics, like acquisition or churn. It's pretty clear that price or ETA are universally important. Including a broader set of market-specific features may reveal insights for expansion. 
