...
Manus initially used a `todo.md` for task planning, but found that roughly one-third of all actions were spent updating the todo list, wasting valuable tokens. They shifted to a  dedicate planner agent that calls sub-agents (executors) for tasks via function calls.
...
For more complex tasks (e.g., the sub-agent needs to write to files that the parent also uses), the planner shares its *full* context with the executor sub-agent. The sub-agent still has its own action space (tools) and prompt, but receives the full context of the planner. 
...
Rather than bloating the function calling layer, Manus pushes many actions to the sandbox layer. This includes many utilities that can be executed using the shell tool. Interestingly, even [MCP tools](https://modelcontextprotocol.io/docs/getting-started/intro) are using the shell tool and an MCP CLI.

This is subtle but important. The design means that the LLM *only* need to know how to use the shell tool effectively for the above actions. It limits the number of functions that the LLM can call, reducing cognitive load and saving tokens in the agent instructions. 

In addition, Manus has access to a filesystem. As explained above, Manus offloads context (e.g., tool results) to the filesystem. Like Claude Code, Manus uses basic utilities (e.g., `glob` and `grep`) to search the filesystem without the need for indexing or vectorstores. Peak mentioned that indexing is useful for (larger) document collections (e.g., enterprise knowledge bases, etc) and can be connected to Manus via MCP.
