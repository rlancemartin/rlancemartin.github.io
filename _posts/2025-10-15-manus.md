...
Manus initially used a `todo.md` for task planning, but found that roughly one-third of all actions were spent updating the todo list, wasting valuable tokens. They shifted to a sub-agent approach. A dedicate planner agent just calls a sub-agents (executors) via function calls.
...
For more complex tasks (e.g., the sub-agent needs to write to files that the parent also uses), the planner shares its *full* context with the executor sub-agent. The sub-agent still has its own action space (tools) and prompt, but receives the full context of the parent (e.g., the full conversation history). 
...
These actions are executed in the sandbox. The key point is this: Manus pushes many actions to the sandbox layer, rather than the function calling layer. Specifically, it includes many utilities (e.g., speech tools, MCP CLI, etc) as shell commands. The agent's instructions briefly mention that utilities are available and instructs the agent to use commands (e.g., `--help` flags) to learn about them.
