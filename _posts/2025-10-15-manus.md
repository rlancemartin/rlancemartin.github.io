...
#TODO(Claude): review to ensure ensure terminology is correct. For example, there is a shell tool that allows agent to execute arbitrary shell commands, which includes the MCP CLI.

Rather than bloating the function calling layer, Manus pushes many actions to the sandbox layer as utilities that can be executed (e.g., with the shell tool). Even [MCP tools](https://modelcontextprotocol.io/docs/getting-started/intro) use a CLI that can be executed with the shell tool.
...
Manus's layer actions space means that even as we add more MCP tools, the LLM *only* need to know how to use the single shell tool to execute them. It limits the number of functions that the LLM can call, reducing cognitive load and saving tokens in the agent instructions. 
